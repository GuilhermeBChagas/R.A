            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-lg font-black text-slate-800 dark:text-slate-100 uppercase leading-none">
                  {filterStatus === 'PENDING' ? 'Pendentes' : 'Histórico de Atendimentos'}
                </h2>
...
export function App() {
  ...
  const renderContent = () => {
    switch (view) {
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-xl">
                    {pendingSubTab === 'INCIDENTS' ? <FileText size={20} className="text-blue-600 dark:text-blue-400" /> : <ArrowRightLeft size={20} className="text-blue-600 dark:text-blue-400" />}
                  </div>
                  <div>
                    <h2 className="text-lg font-black text-slate-800 dark:text-slate-100 uppercase tracking-wide">
                      {pendingSubTab === 'INCIDENTS' ? 'Atendimentos Pendentes' : 'Cautelas Pendentes'}
                    </h2>
                    <p className="text-xs text-slate-500">
                      {pendingSubTab === 'INCIDENTS' ? 'Aguardando validação ou recebimento' : 'Aguardando confirmação de recebimento'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {pendingSubTab === 'INCIDENTS' && pendingIncidentsCount > 0 && (
                    <span className="px-3 py-1 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-full text-xs font-black">{pendingIncidentsCount} pendente(s)</span>
                  )}
                  {pendingSubTab === 'LOANS' && pendingLoansCount > 0 && (
                    <span className="px-3 py-1 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-full text-xs font-black">{pendingLoansCount} pendente(s)</span>
                  )}
                </div>
              </div>
            </div>
      ...
      case 'LOANS': return <LoanViews currentUser={user!} users={users} vehicles={vehicles} vests={vests} radios={radios} equipments={equipments} onLogAction={createLog} loans={loans} onRefresh={() => fetchLoans(false)} />;
        ...
        <div className="h-full flex flex-col text-white">
          ...
          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            <div className="pt-4 pb-2 border-t border-brand-800">
              ...
              <div className="relative">
                <NavItem icon={<UserCheck />} label="Pendentes" active={view === 'PENDING_APPROVALS'} onClick={() => setPendingMenuOpen(!pendingMenuOpen)} collapsed={isSidebarCollapsed} badge={totalPendingBadge > 0 ? totalPendingBadge : undefined} />
                {!isSidebarCollapsed && <div className="absolute right-8 top-3.5 pointer-events-none text-brand-300">{pendingMenuOpen ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</div>}
              </div>
              {pendingMenuOpen && !isSidebarCollapsed && (
                <div className="space-y-1 mt-1">
