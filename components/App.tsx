...
const NavItem: React.FC<NavItemProps> = ({ icon, label, active, onClick, collapsed, badge, isSubItem }) => (
  ...
  >
    ...
    {badge && badge > 0 && (
      <span className={`absolute ${collapsed ? 'top-1 right-1' : 'right-8'} badge-pulse bg-red-500 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full`}>
        {badge}
      </span>
    )}
  </button>
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <h2 className="text-xl md:text-2xl font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight flex items-center gap-2">
                <UserCheck className="text-blue-600" /> {pendingSubTab === 'INCIDENTS' ? 'Pendências de Atendimento' : 'Pendências de Cautelas'}
              </h2>
            </div>

            {/* Content based on sub-tab selected from sidebar */}
          ...
          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            <div className="pt-4 pb-2 border-t border-brand-800">
              ...
              <div className="relative">
                ...
                {!isSidebarCollapsed && <div className="absolute right-3 top-3.5 pointer-events-none text-brand-300">{pendingMenuOpen ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</div>}
              </div>
              {pendingMenuOpen && !isSidebarCollapsed && (
                <div className="space-y-1 mt-1">
                  <NavItem label="Atendimento" icon={<FileText size={14} className="mr-2" />} active={view === 'PENDING_APPROVALS' && pendingSubTab === 'INCIDENTS'} onClick={() => { setPendingSubTab('INCIDENTS'); handleNavigate('PENDING_APPROVALS'); }} collapsed={isSidebarCollapsed} isSubItem badge={pendingIncidentsCount > 0 ? pendingIncidentsCount : undefined} />
